...
async function writeRow(tencentDocsConfig, sessionId, fileId, sheetId, startRow, values) {
  const cellValues = values.map((val, idx) => ({
    row: startRow, col: idx, value_type: 'STRING', string_value: String(val)
  }));
...
async function extractRowData(llmConfig, headers, tableName, userDescription) {
  ...
  return { values: values, missing: missing, raw: raw };
}
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/write/extract' && request.method === 'POST') {
      ...
      try {
        ...
        return jsonResponse({
          success: true,
          data: { headers: headers, values: values, missing: missing, targetRow: emptyRowIndex, sheetName: sheet.sheet_name, sheetId: sheet.sheet_id, targetFileId: targetFileId, preview: buildPreviewText(headers, values) }
        });
      } catch (err) {
        return jsonResponse({ success: false, error: err.message }, 500);
      }
    ...
    if (url.pathname === '/api/write/execute' && request.method === 'POST') {
      ...
      try {
        ...
        if (checkLines.length > targetRow + 1) {
          const rowCells = parseCsvLine(checkLines[targetRow + 1] || '');
